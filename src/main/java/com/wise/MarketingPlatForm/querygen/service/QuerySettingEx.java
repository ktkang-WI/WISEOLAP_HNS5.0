...
import com.wise.MarketingPlatForm.querygen.dto.Comparison;
import com.wise.MarketingPlatForm.querygen.dto.Condition;
import com.wise.MarketingPlatForm.querygen.dto.DimensionTbl;
import com.wise.MarketingPlatForm.querygen.dto.DsViewTbl;
import com.wise.MarketingPlatForm.querygen.dto.Hierarchy;
import com.wise.MarketingPlatForm.querygen.dto.JoinType;
import com.wise.MarketingPlatForm.querygen.dto.LogicOperator;
import com.wise.MarketingPlatForm.querygen.dto.Measure;
...
import com.wise.MarketingPlatForm.querygen.querysetting.dbms.NETEZZASetting;
...
public class QuerySettingEx {
				...
				if (!aJoinClauseColl.contains(oDr.getTBL_NM())) {
					if (!oNotJoinTbl.contains(oDr.getTBL_NM())) {
						...
						if (sNonJoinTbl.equals(""))
							sNonJoinTbl = this.owner+oDr.getTBL_NM();
						else
							sNonJoinTbl = sNonJoinTbl + "," + this.owner+oDr.getTBL_NM();
	...
	public String ColumnAliasNm(String aDbmsType, String aColumnNm) {
		...
		if (aDbmsType.equalsIgnoreCase("IMPALA") || aDbmsType.equalsIgnoreCase("MARIA")
				|| aDbmsType.equalsIgnoreCase("Maria") || aDbmsType.equalsIgnoreCase("MYSQL")) {
			IMPALASetting oSetting = new IMPALASetting();
			sAliasNm = oSetting.ColumnAliasNm(aColumnNm);
		}
//		if (aDbmsType.equalsIgnoreCase("POSTGRES")) {
//			POSTGRESSetting oSetting = new POSTGRESSetting();
//			sAliasNm = oSetting.ColumnAliasNm(aColumnNm);
//		}
		return sAliasNm;
	}
		...
		for (SelectCube oDr : aDtSel) {
			if ("MEA".equalsIgnoreCase(oDr.getTYPE())) {
				...
				if (oRows.size() > 0) {
					if (oRows.get(0).getMEA_GRP_UNI_NM().equalsIgnoreCase(aMeaGrpUniNm)) {
						...
						if (sColExpress != null && !sColExpress.equals("")) {
							...
							// if(oRows.get(0).getMEA_AGG().equalsIgnoreCase("Distinct Count"))
							// oSelClauseColl.add("Count" + "(Distinct " + oTblNm + "." + oColNm + ")" + "
							// AS " + oColCaption);
							// else if(oRows.get(0).getMEA_AGG().equalsIgnoreCase(""))
				...
				if (oRows.size() > 0) {
					 ...
					 * sKeyValue_K = oAliasRows.get(0).getALIAS_TBL_NM() + "." +
					 * oRows.get(1).getCOL_NM() + " AS " + ColumnAliasNm(aDBMSType,
					 * oDr.getCAPTION()+ "_K");
					 * else
					 * sKeyValue_K = sTblNm + "." + oRows.get(1).getCOL_NM() + " AS " +
						...
						if (oAliasRows.size() > 0)
							for (int i = 0; i < oAliasRows.size(); i++) {
								if (sTblNm.equalsIgnoreCase(oAliasRows.get(i).getTBL_NM())
									...
									sKeyValue = oAliasRows.get(i).getALIAS_TBL_NM() + "."
				...
				if (oRows.size() > 0) {
					if (oRows.get(0).getMEA_GRP_UNI_NM().equalsIgnoreCase(aMeaGrpUniNm)) {
						...
						if (sColExpress != null && !sColExpress.equals("")) {
							...
							if (oRows.get(0).getMEA_AGG() != null
									&& oRows.get(0).getMEA_AGG().trim().equalsIgnoreCase("Distinct Count"))
								oSelClauseColl.add(
										"Count" + "(Distinct " + oTblNm + "." + oColNm + ")" + " AS " + oColCaption);
							else if (oRows.get(0).getMEA_AGG() != null
									&& oRows.get(0).getMEA_AGG().equalsIgnoreCase(""))
		...
		for (SelectCube oDr : aDtSel) {
			if ("MEA".equalsIgnoreCase(oDr.getTYPE())) {
				...
				if (oRows.size() > 0) {
					 ...
					 */
					if (sColExpress != null && !sColExpress.equals("")) {
						/* dogfoot 주제영역 사용자 컬럼 추가 시 group BY 에 들어가는 오류 수정 shlim 20210331 */
						// if (oRows.get(0).getMEA_AGG().trim().equalsIgnoreCase("Distinct Count"))
						// oGroupByColl.add("Count" + "(Distinct " + sColExpress + ")" + " AS " +
						// oColCaption);
						// else
						...
						if (oRows.get(0).getMEA_AGG() != null
								&& oRows.get(0).getMEA_AGG().trim().equalsIgnoreCase("")) {
							/* DOGFOOT 단일테이블에서 칼럼에 괄호 포함 시 조회 안되는 오류 수정 syjin 20210524 */
							// if(oDr.getDATA_TYPE().equals("cal") || oColNm.indexOf("(") > -1 ||
							// oColNm.indexOf(")") > -1 || (oColNm.toLowerCase().indexOf("case") > -1 &&
							// oColNm.toLowerCase().indexOf("when") > -1)) {
							if (oDr.getDATA_TYPE().equals("cal") || (oColNm.toLowerCase().indexOf("case") > -1
									&& oColNm.toLowerCase().indexOf("when") > -1)) {
	...
	public String CubeOrderByClause(String aDBMSType,
		...
		if (aDBMSType.equalsIgnoreCase("MS-SQL") && !sOrderBy.equals(""))
	...
	public ArrayList<String> CubeUniOnSelClause(String aDBMSType,
		...
		for (SelectCube oDr : aDtSel) {
			...
			if (oDr.getTYPE().equalsIgnoreCase("MEA")) {
				...
				oSelClauseColl.add("Sum" + "(" + ColumnAliasNm(aDBMSType, sCaption) + ") AS "
						+ ColumnAliasNm(aDBMSType, sCaption));
				...
				// if(oRows.size() != 1)
				// oSelClauseColl.add(ColumnAliasNm(aDBMSType, sCaption) + " AS " +
				// ColumnAliasNm(aDBMSType, sCaption) + "_K");
			}
		}
		return oSelClauseColl;
	}
	...
	// 20210810 AJKIM 정렬 컬럼 오류 수정 dogfoot
	public ArrayList<String> CubeUniOnGroupByByClause(String aDBMSType, ArrayList<Hierarchy> aDtSelHie,
			ArrayList<SelectCube> aDtSel) {
		ArrayList<String> oGroupByColl = new ArrayList<String>();
		...
		for (SelectCube oDr : aDtSel) {
			if (oDr.getTYPE().equals("DIM"))
				oGroupByColl.add(ColumnAliasNm(aDBMSType, oDr.getCAPTION()));
		}
	...
	// 20210511 AJKIM 차원 멤버 groupBy 절에서 제거하기 위해 CubeQuerySetting 오버로딩
	...
	public String CubeQuerySetting(ArrayList<SelectCube> aDtSel,
		...
		if (!oMeaGrpColl.isEmpty()) {
			for (String sMeaGrpNm : oMeaGrpColl) {
				...
				oQueryGen.SelectFromTable(owner+sMeaTblNm);
				...
				ArrayList<Hierarchy> oDtSelHieCopy = aDtSelHIeJoin;
				...
				for (Relation oDr : oDtJoin) {
					...
					String ownerWithPkTblNm = "";
					String ownerWithFkTblNm = "";
					...
					if (oFkTblAliasRows.size() > 0)
						oQueryGen.AddJoin(JoinType.getJoinType(oDr.getJOIN_TYPE()),
								owner + oDr.getPK_TBL_NM() + " " + sPkTblAlias,
								sPkTblAlias + "." + ColumnAliasNm(aDBMSType, oDr.getPK_COL_NM()), Comparison.Equals,
								owner + oDr.getFK_TBL_NM(), oFkTblAliasRows.get(0).getALIAS_TBL_NM() + "."
										+ ColumnAliasNm(aDBMSType, oDr.getFK_COL_NM()));
					else {
						// PK & FK 설정
						...
						if (oDr.getEXPRESSION() != null) {
							for (Relation r : aDtCubeRel) {
								if (r.getFK_COL_NM().equals(oDr.getFK_COL_NM())) {
									oQueryGen.AddJoin(JoinType.getJoinType(oDr.getJOIN_TYPE()),
											owner + oDr.getPK_TBL_NM() + " " + sPkTblAlias,
											sPkTblAlias + "." + ColumnAliasNm(aDBMSType, oDr.getPK_COL_NM()),
											Comparison.Equals, owner + oDr.getFK_TBL_NM(), oDr.getEXPRESSION());
									break;
								} else if (r.getPK_COL_NM().equals(oDr.getPK_COL_NM())) {
									oQueryGen.AddJoin(JoinType.getJoinType(oDr.getJOIN_TYPE()),
											owner + oDr.getPK_TBL_NM() + " " + sPkTblAlias, oDr.getEXPRESSION(),
											Comparison.Equals, owner + oDr.getFK_TBL_NM(),
						...
						// if(r.getFK_COL_NM().equals(oDr.getFK_COL_NM())) {
						// oQueryGen.AddJoin(JoinType.getJoinType(oDr.getJOIN_TYPE()),
						// oDr.getPK_TBL_NM() + " " + sPkTblAlias , sPkTblAlias + "." +
						// ColumnAliasNm(aDBMSType, oDr.getPK_COL_NM()) , Comparison.Equals,
						// oDr.getFK_TBL_NM(), oDr.getEXPRESSION());
						// break;
						// }
						// }
						// }
						...
						// oQueryGen.AddJoin(JoinType.getJoinType(oDr.getJOIN_TYPE()),
						// oDr.getPK_TBL_NM() + " " + sPkTblAlias , oDr.getEXPRESSION() ,
						// Comparison.Equals, oDr.getFK_TBL_NM(), oDr.getFK_TBL_NM() + "." +
						// ColumnAliasNm(aDBMSType, oDr.getFK_COL_NM()));
						// break;
						// }
						// }
						// }
							...
							// oDr.getPK_TBL_NM() + " " + sPkTblAlias , sPkTblAlias + "." +
							// ColumnAliasNm(aDBMSType, oDr.getPK_COL_NM()) , Comparison.Equals,
							// oDr.getFK_TBL_NM(), oDr.getFK_TBL_NM() + "." + ColumnAliasNm(aDBMSType,
							// oDr.getFK_COL_NM()));
							...
							oQueryGen.AddJoin(JoinType.getJoinType(oDr.getJOIN_TYPE()),
									owner + oDr.getPK_TBL_NM() + " " + sPkTblAlias, PK_VALUE, Comparison.Equals,
									owner + oDr.getFK_TBL_NM(), FK_VALUE);
				...
				String sNonJoinTbl = CubeNonRelTbl(oDtSelHieCopy, oDtWhereCopy, oDtOrderCopy, oJoinClauseColl,
						sMeaTblNm, oMeaGrpColl);
				...
				for (SelectCubeWhere oDr : aDtWhere) {
					...
					if (oDr.getAGG().equalsIgnoreCase("")) {
						...
						/* dogfoot 주제영역 복사된 테이블 필터 올릴시 알리아스 오류 수정 shlim 20210503 */
						if (oAliasRows.size() > 0) {
							if (oAliasRows.size() > 1) {
								for (TblAlias oAliasRow : oAliasRows) {
									if (sTblNm.equalsIgnoreCase(oAliasRow.getTBL_NM())
											&& oDr.getPARENT_UNI_NM().equalsIgnoreCase(oAliasRow.getUNI_NM()))
						...
						// 20210409 AJKIM 상세데이터 조회 오류 수정 dogfoot
						...
						if (sOper == null || sOper.equals(null) || sOper.equalsIgnoreCase(""))// ||
																								// sOper.equalsIgnoreCase("LIST"))
							sOper = "In";
						...
						if (Boolean.parseBoolean(oDr.getPARAM_YN()))
							...
							if (oDr.getVALUES().equalsIgnoreCase("")) {
								/* dogfoot 주제영역 계산된 컬럼 필터 올릴시 치환 추가 shlim 20210408 */
								if (!sColExpress.equals("")) {
									sValue = sColExpress;
								} else {
								...
								if("wise_null_value".equals(oDr.getVALUES())) {
									sValue = "";
								}else {
							...
							if (oDr.getVALUES().equalsIgnoreCase("[All]")
								...
								if (sValue.indexOf("[_ALL_VALUE_]") >= 0) {
									...
									sValue = sValueArr[0].replace("[[", "").replace("]", "").replace("_ALL_VALUE_",
											oDr.getWHERE_CLAUSE());
									sValue += ",";
									sValue += sValueArr[1].replace("[", "").replace("]]", "").replace("_ALL_VALUE_",
											oDr.getWHERE_CLAUSE());
								}
								if (sValue.indexOf("[_EMPTY_VALUE_]") >= 0) {
									String[] sValueArr = sValue.split(",");
									sValue = sValueArr[0].replace("[[", "").replace("]", "").replace("_EMPTY_VALUE_",
											oDr.getWHERE_CLAUSE());
									sValue += ",";
									sValue += sValueArr[1].replace("[", "").replace("]]", "").replace("_EMPTY_VALUE_",
											oDr.getWHERE_CLAUSE());
								}
						...
						oQueryGen.AddWhere(sFieldNm, Comparison.getComparisonType(sOper), CheckValue(sValue), 1,
				...
				if (aDtWhere.size() > 0) {
					if (sChnCond.equals("")) {
						...
						if (!sWhereClauseScript.equals(""))
							sWhereClauseScript = "WHERE" + "   " + sChnCond + vbCrLf + "AND" + "      "
									+ sWhereClauseScript;
						else
...
//						aDtSelMea);
				...
				if("HISTOGRAM_CHART".equals(itemType) || "BOX_PLOT".equals(itemType)) {
					 ...
					 oGroupByColl = CubeGroupByByClause(aDBMSType, aDtSelHIe, oDtTblAlias, aDtSel,
								aDtSelMea);
				}
				...
				for (SelectCubeWhere oDr : aDtWhere) {
					...
					if (!oDr.getAGG().equals("")) {
						...
						oQueryGen.AddHaving(sHavingFldNm, Comparison.getComparisonType(sOper), sValue, 1, sDataType,
								oDr.getPARAM_NM());
					}
			...
			if (oUnionClauseColl.size() > 1) {
				...
				ArrayList<String> oGroupByColl = CubeUniOnGroupByByClause(aDBMSType, aDtSelHIe, aDtSel);
			...
			ArrayList<SelectCubeWhere> oDtWhereCopy = aDtWhere;
			ArrayList<SelectCubeOrder> oDtOrderCopy = aDtOrder;
			ArrayList<Relation> oDtJoin = CubeJoinSetting(oDtSelHieCopy, oDtSelMeaCopy, oDtWhereCopy, oDtOrderCopy,
					aDtCubeRel, aDtDsViewRel, true);
			...
			if("".equals(sFromTblNm)) {
				oQueryGen.SelectFromTable(sFromTblNm);
			}else {
				oQueryGen.SelectFromTable(owner+sFromTblNm);
			}
			sFromClauseScript = oQueryGen.BuildQueryFrom();
			...
			String sNonJoinTbl = CubeNonRelTbl(oDtSelHieCopy, oDtWhereCopy, oDtOrderCopy, oJoinClauseColl, sFromTblNm,
					oMeaGrpColl);
			...
			if (!sNonJoinTbl.equals("")) {
				...
				if (sFromTblNm.equals(""))
					sFromClauseScript = sFromClauseScript
							+ ChkSingleTbl(oDtSelHieCopy, oDtWhereCopy, oDtOrderCopy, oJoinClauseColl);
			}
			...
			for (SelectCubeWhere oDr : aDtWhere) {
				...
				if (oDr.getAGG().equalsIgnoreCase("")) {
					...
					oQueryGen.AddWhere(sFieldNm, Comparison.getComparisonType(sOper), CheckValue(sValue), 1, sDataType,
							oDr.getPARAM_NM(), LogicOperator.getLogicOperator(sLogic));
					String sTmpWhereClauseScript = oQueryGen.BuildQueryOnlyWhere();
					sTmpWhereClauseScript = sTmpWhereClauseScript.replace("AND" + "\t", "").trim();
					...
					if (sChndCond.equals("") || sCondId.equals("")) {
						if (sWhereClauseScript.equals(""))
							sWhereClauseScript = sTmpWhereClauseScript;
						else
							sWhereClauseScript = sWhereClauseScript + vbCrLf + "AND     " + sTmpWhereClauseScript;
			...
			if (aDtWhere.size() > 0) {
				if (sChndCond.equals("")) {
					...
					if (!sWhereClauseScript.equals(""))
						sWhereClauseScript = "WHERE" + "   " + sChndCond + vbCrLf + "AND" + "      "
								+ sWhereClauseScript;
					else
			...
			ArrayList<String> oGroupByColl = CubeGroupByByClause(aDBMSType, aDtSelHIe, new ArrayList<TblAlias>(),
					aDtSel, aDtSelMea);
			// ArrayList<String> oGroupByColl = CubeGroupByByClause(aDBMSType,aDtSelHIe,new
			// ArrayList<TblAlias> ());
			if (oGroupByColl.size() > 0) {
				oQueryGen.GroupBy(oGroupByColl);
				sGroupByClauseScript = oQueryGen.BuildQueryGroupBy();
		...
		if (!oMeaGrpColl.isEmpty()) {
			for (String sMeaGrpNm : oMeaGrpColl) {
				...
				for (Relation oDr : oDtJoin) {
					...
					if (oFkTblAliasRows.size() > 0)
						oQueryGen.AddJoin(JoinType.getJoinType(oDr.getJOIN_TYPE()),
								oDr.getPK_TBL_NM() + " " + sPkTblAlias,
								sPkTblAlias + "." + ColumnAliasNm(aDBMSType, oDr.getPK_COL_NM()), Comparison.Equals,
								oDr.getFK_TBL_NM(), oFkTblAliasRows.get(0).getALIAS_TBL_NM() + "."
										+ ColumnAliasNm(aDBMSType, oDr.getFK_COL_NM()));
					else {
						// PK & FK 설정
							...
							for (Relation r : aDtCubeRel) {
								if (r.getFK_COL_NM().equals(oDr.getFK_COL_NM())) {
									oQueryGen.AddJoin(JoinType.getJoinType(oDr.getJOIN_TYPE()),
											oDr.getPK_TBL_NM() + " " + sPkTblAlias,
											sPkTblAlias + "." + ColumnAliasNm(aDBMSType, oDr.getPK_COL_NM()),
											Comparison.Equals, oDr.getFK_TBL_NM(), oDr.getEXPRESSION());
									break;
								} else if (r.getPK_COL_NM().equals(oDr.getPK_COL_NM())) {
									oQueryGen.AddJoin(JoinType.getJoinType(oDr.getJOIN_TYPE()),
											oDr.getPK_TBL_NM() + " " + sPkTblAlias, oDr.getEXPRESSION(),
											Comparison.Equals, oDr.getFK_TBL_NM(),
											oDr.getFK_TBL_NM() + "." + ColumnAliasNm(aDBMSType, oDr.getFK_COL_NM()));
									break;
								}
							}
					...
					if (oDr.getAGG().equalsIgnoreCase("")) {
						...
						if (Boolean.getBoolean(oDr.getPARAM_YN()))
							...
							if (oDr.getVALUES().equalsIgnoreCase("[All]")
								...
								if (sOper.equalsIgnoreCase("Between") || sOper.equalsIgnoreCase("BETWEEN_CAND")
										|| sOper.equalsIgnoreCase("BETWEEN_LIST")
										|| sOper.equalsIgnoreCase("BETWEEN_INPUT")) {
									if (sWhereClause.equalsIgnoreCase(""))
										sValue = sFieldNm + "," + sFieldNm;
									else
										sValue = sWhereClause + "," + sWhereClause;
	...
	public String CubeQuerySettingSingleDS(ArrayList<SelectCube> aDtSel,
		...
		if (!oMeaGrpColl.isEmpty()) {
			for (String sMeaGrpNm : oMeaGrpColl) {
				...
				// ---------------------------------------------------------------------'
				oSelClauseCollRename = CubeSelClauseForRenameTable(aDBMSType, aDtSel, aDtSelHIe, aDtSelMea, sMeaGrpNm,
						oDtTblAlias);
				...
				// GROUP BY 절 종료
				// ---------------------------------------------------------------------'
				System.out.println(sSelCaluseScriptForRename);
			...
			// SELECT A.COL_NM 절 시작
			// ---------------------------------------------------------------------'
			oSelClauseCollRename = CubeSelClauseForRenameTable(aDBMSType, aDtSel, aDtSelHIe, aDtSelMea, "",
					new ArrayList<TblAlias>());
