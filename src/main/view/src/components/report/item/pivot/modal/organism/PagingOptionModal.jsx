...
const PagingOptionModal = ({options, onSubmit, onClose}) => {
  const [pagination, setPagination] = useState(options.pagination);
  const [pageUsageOfPageCount, setPageUsageOfPageCount] =
    useState(options.pageUsageOfPageCount);
  ...
  return (
    <Modal
      onSubmit={() => {
        onSubmit({paging:
          {pagination, pageUsageOfPageCount}});
        return false;
      }}
      onClose={onClose}
...
export default PagingOptionModal;
